document.getElementById('uploadForm').addEventListener('submit', async function(event) {
    ...
    try {
        ...
        console.log('Status da resposta:', response.status); // Adicione este log para verificar o status

        if (!response.ok) {
            const errorMessage = await response.text(); // Captura a mensagem de erro
            throw new Error(`Erro ao enviar a imagem: ${errorMessage}`);
        }
    ...
    try {
        ...
        posts.forEach(post => {
            ...
            postDiv.innerHTML = `
                ...
                <p>${post.descricao || 'Descrição não disponível.'}</p>
            `;
